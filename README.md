...
- The browser requests terms and song suggestions directly from `https://api.music.apple.com/v1/catalog/us/search/suggestions`, debounced by 250 ms.
- The browser discovers songs directly through `https://api.music.apple.com/v1/catalog/us/search`, with lyric snippets, artwork, ISRCs, and pagination. It only labels `meta.snippets` of kind `lyric` as lyric matches. Superseded catalog requests are cancelled.
- Credentials come from `https://am-mint.binimum.org/token` directly in the browser. Token fetches are coalesced per page, cached in memory below the supplied TTL (capped at 120 seconds), and refreshed once after a 401/403. Tokens are obtained at runtime, never embedded in source code or persisted in browser storage. Only the official Apple Music API host is used for catalog requests, with the browser supplying its own Origin header. The catalog defaults to US; the mint's numeric storefront header is not an ISO country code.
